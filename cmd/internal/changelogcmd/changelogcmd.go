...
// read files → model.Run → atomic write (log first, then anchor). Shared by the
// bin/pair-changelog shim and the `pair changelog` dispatcher route.
...
// Run distills the cleaned TTY into the change log, streaming live per-batch
...
func run(args []string, stderr io.Writer) error {
	...
	if len(content) == 0 {
		return nil // nothing captured yet; leave the log untouched
	}
	...
	if hasPrior && res.Kind != FullRedistill && len(boundaries) <= priorTurns {
		fmt.Fprintln(stderr, "pair-changelog: up to date (no new turn)")
		return nil
	}

	if res.Kind == NoOp {
		return nil // belt-and-suspenders; shouldn't fire once a new turn exists
	}
	...
	for _, seg := range splitByDate(slice, sliceDates) {
		...
		for i, chunk := range chunks {
			if len(chunks) > 1 {
				fmt.Fprintf(stderr, "pair-changelog: distilling batch %d/%d (%d lines)\n", i+1, len(chunks), len(chunk))
			} else {
				fmt.Fprintf(stderr, "pair-changelog: distilling %d lines\n", len(chunk))
			}
			...
			if err := atomicWrite(logPath, newLog); err != nil {
				return fmt.Errorf("write log: %v", err)
			}
		}
	}
	...
	if err := writeAnchor(anchorPath, len(boundaries), anchorSnippet(content, anchorLines)); err != nil {
		return fmt.Errorf("write anchor: %v", err)
	}
	if newLog == priorLog {
		return nil // processed, but no new entry → anchor advanced; nothing to flash
	}
