...
//
// Packaging: the logic lives here in cmd/internal/wrapcmd behind Run(); it is
// reached two ways with byte-for-byte identical behavior — the standalone
// bin/pair-wrap shim (cmd/pair-wrap) and the `pair wrap` dispatcher route
// (#96, on #92's streaming seam). Both pass the real os.Stdin/os.Stdout so the
// io.Reader→*os.File assertion in Run always succeeds in production.
package wrapcmd
...
func (p *proxy) translateStdin() {
	p.translateStdinFrom(p.stdin, p.ptmx, pendingFlushAfter)
}
...
func (p *proxy) writeStartupBanner() {
	var cols int
	if p.stdoutFile != nil {
		if ws, err := pty.GetsizeFull(p.stdoutFile); err == nil {
			cols = int(ws.Cols)
		}
	}
	if cols == 0 {
...
func run(args []string, stdin io.Reader, stdout, stderr io.Writer) (int, error) {
	...
	for len(argv) > 0 && strings.HasPrefix(argv[0], "-") {
		...
		case argv[0] == "--":
			...
			goto argsDone
		default:
			return 0, fmt.Errorf("unknown flag %q", argv[0])
		}
	}
argsDone:
	if len(argv) == 0 {
	...
	if p.stdinFile != nil && isTTY(p.stdinFile) {
		s, err := term.MakeRaw(int(p.stdinFile.Fd()))
		if err == nil {
			oldState = s
		}
	}
	defer func() {
		if oldState != nil {
			_ = term.Restore(int(p.stdinFile.Fd()), oldState)
		}
