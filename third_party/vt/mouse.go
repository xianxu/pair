...
func (e *Emulator) SendMouse(m Mouse) {
	...
	if isMotion && (mode != ansi.ModeMouseAnyEvent && (mode != ansi.ModeMouseButtonEvent || mouse.Button == MouseNone)) {
		return
	}
	if isRelease && mode == ansi.ModeMouseX10 {
		return
	}
	...
	case nil: // X10 mouse encoding
		...
		if mouse.X < 0 || mouse.Y < 0 || mouse.X > 222 || mouse.Y > 222 {
			return
		}
...
var trackingModes = []ansi.DECMode{ansi.ModeMouseX10, ansi.ModeMouseNormal, ansi.ModeMouseHighlight, ansi.ModeMouseButtonEvent, ansi.ModeMouseAnyEvent}

func mouseTrackingMode(m ansi.Mode) bool {
	for _, v := range trackingModes {
		if m == v {
			return true
		}
	}
