...
type Emulator struct {
	...
	replyError         error
	keyboard           [2]keyboardState
	cluster            string
	clusterX, clusterY int
	clusterWidth       int
	clusterOld         [2]uv.Cell
...
func NewEmulator(w, h int) *Emulator {
	l := DefaultLimits()
	if !l.geometry(w, h) {
		panic("vt: invalid geometry")
	}
	return newEmulator(w, h, l)
}

func newEmulator(w, h int, limits Limits) *Emulator {
	t := new(Emulator)
	t.limits = limits
...
func (e *Emulator) Resize(width int, height int) { _ = e.ResizeChecked(width, height) }
...
func (e *Emulator) SetScrollbackSize(maxLines int) {
	if maxLines > e.limits.HistoryLines {
		maxLines = e.limits.HistoryLines
	}
