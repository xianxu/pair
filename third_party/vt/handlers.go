...
func (e *Emulator) registerDefaultHandlers() {
	e.registerKeyboardHandlers()
	e.RegisterCsiHandler('u', func(ansi.Params) bool { e.restoreCursor(); return true })
...
func (e *Emulator) registerDefaultCsiHandlers() {
	...
	e.RegisterCsiHandler('J', func(params ansi.Params) bool {
		...
		case 0: // Erase screen below (including cursor)
			rect1 := uv.Rect(x, y, width-x, 1)          // cursor to end of line
		...
		case 3: // xterm: erase saved lines, leaving visible cells unchanged
	...
	e.RegisterCsiHandler('c', func(params ansi.Params) bool {
		...
		_, _ = io.WriteString(e.replies(), ansi.PrimaryDeviceAttributes(
			62, // VT220
			1,  // 132 columns
			6,  // Selective Erase
			22, // ANSI color
	...
	e.RegisterCsiHandler(ansi.Command('>', 0, 'c'), func(params ansi.Params) bool {
		...
		// Do we fully support VT220?
		_, _ = io.WriteString(e.replies(), ansi.SecondaryDeviceAttributes(
			1,  // VT220
			10, // Version 1.0
	...
	e.RegisterCsiHandler('n', func(params ansi.Params) bool {
		...
		case 5: // Operating Status
			...
			_, _ = io.WriteString(e.replies(), "\x1b[0n")
		case 6: // Cursor Position Report [ansi.CPR]
			x, y := e.scr.CursorPosition()
			_, _ = io.WriteString(e.replies(), ansi.CursorPositionReport(y+1, x+1))
		default:
			return false
		}
	...
	e.RegisterCsiHandler(ansi.Command('?', 0, 'n'), func(params ansi.Params) bool {
		...
		case 6: // Extended Cursor Position Report [ansi.DECXCPR]
			x, y := e.scr.CursorPosition()
			_, _ = io.WriteString(e.replies(), ansi.ExtendedCursorPositionReport(y+1, x+1, 0)) // We don't support page numbers //nolint:errcheck
		default:
			return false
		}
	...
	e.RegisterCsiHandler('r', func(params ansi.Params) bool {
		...
		if top >= bottom || top > height || bottom > height {
			return false
		}
