...
| **Return** | agent pane | Insert a newline in the agent's composer, matching the draft pane. For `codex`, `muse`, and `agy` the rewrite is *positively gated*: Pair rewrites Return only while it can see a live composer on screen, so in a permission picker, a selection menu, or any state it doesn't recognize, Return stays a plain Enter and the dialog confirms. `claude` rewrites unconditionally and relies on its permission notification to suspend the remap. Set `PAIR_WRAP_REMAP_RETURN=0` to turn the rewrite off entirely. |
