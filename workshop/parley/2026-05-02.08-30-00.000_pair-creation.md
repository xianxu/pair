---
topic: pair-creation
file: 2026-05-02.08-30-00.000_pair-creation.md
tags: pair, zellij, nvim, claude-code, tui, design-conversation
note: Reconstructed transcript from the Claude Code chat session that produced pair v1. User turns are verbatim; assistant turns reproduce the actual code shipped and the substance of the explanations, but long prose is condensed where it doesn't add to the record.
---
...
Yes — but "inject into the input window" is really "write bytes into the PTY master that the TUI is reading from." There are four levels, ranked by how well they actually work.

### 1. tmux / screen — the practical winner

If the TUI runs inside a tmux pane, tmux *is* the PTY master and exposes an API:

```bash
tmux send-keys -t claude:0.0 "summarize the failing test" 
tmux send-keys -t claude:0.0 Enter
```

Or as literal bytes (safer with special chars):

```bash
tmux send-keys -t claude:0.0 -l "text with -- dashes and ; semicolons"
tmux send-keys -t claude:0.0 C-m
```

GNU screen equivalent: `screen -S sess -X stuff $'text\n'`.

This is definitely how most Claude Code orchestration tooling does it. You also get `tmux capture-pane -p` to read the screen back, so you can build a crude request/response loop. The cost is that you must launch the TUI under tmux.

### 2. Own the PTY yourself

Same idea, no tmux dependency: spawn the TUI under a pseudoterminal you control, then expose a side channel (FIFO, socket) that writes into the master fd.

```python
#!/usr/bin/env python3
"""Run a TUI under a PTY we own; bytes written to $INJECT_FIFO become its input."""
import os, pty, sys, select, fcntl, termios, signal, tty

FIFO = os.environ.get("INJECT_FIFO", "/tmp/tui-inject")


def copy_winsize(dst_fd):
    try:
...
def main(argv):
    ...
    copy_winsize(master)
    signal.signal(signal.SIGWINCH, lambda *_: copy_winsize(master))

    # O_RDWR so the FIFO never sees EOF when a writer disconnects.
    ...
    try:
        ...
        while True:
            try:
                readable, _, _ = select.select([sys.stdin, master, fifo], [], [])
...
if __name__ == "__main__":
    main(sys.argv[1:] or [os.environ.get("SHELL", "/bin/sh")])
```

```bash
# terminal 1
chmod +x ptyinject.py
./ptyinject.py claude

# terminal 2
printf 'what files changed in the last commit?' > /tmp/tui-inject
printf '\r'                                    
