...
- [ ] **Step 2: Run `go test ./cmd/internal/couchtty -run '^TestSwitchTracker' -count=1` and confirm it fails because the type is absent.**

- [ ] **Step 3: Implement the minimal rule.** Nothing beyond the Spec's four lines:
...
- [ ] **Step 3b: Delete `actorAlive` (`console.go:1119`) too.** It exists only as `Up`'s liveness predicate. Go does not report an unused *method*, so the compiler will not find it — this one has to be deleted deliberately.
- [ ] **Step 3c: Confirm the deletion is complete by compilation, not by grep.** `go build ./... && GOOS=linux go build ./...`. A replacement is incomplete while its superseded surface still compiles (`workshop/lessons.md`).
- [ ] **Step 4: Run `go test ./cmd/internal/couchtty -count=1` and confirm it passes.**
- [ ] **Step 5: Commit.** `git commit -m "#170 M1: couch: retire the focus ladder and root actor"`
...
- [ ] **Step 3b: Assert the async site explicitly.** Site 5 fires on the next refresh, not on a keystroke, so a test that only presses keys will pass while the real console drops the confirmation a second later. Drive a refresh between opening the leave confirmation and confirming it.
- [ ] **Step 4: Run `go test ./cmd/internal/couchtty -count=1`; confirm pass.**
- [ ] **Step 5: Commit.** `git commit -m "#170 M1: couch: leave couch from the panel's alt+x"`
...
- [ ] **Step 4: Run the focused tests, then `go test ./cmd/internal/couchtty -count=1`; confirm pass.**
- [ ] **Step 5: Commit.** `git commit -m "#170 M1: couch: intercept ctrl+backspace in both encodings"`
