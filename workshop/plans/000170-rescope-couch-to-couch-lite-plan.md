...
- **`DetachedSessionResolver`** — one method, **`DetachedSessions(ctx context.Context, candidates []DetachedCandidate) ([]DetachedSessionObservation, error)`**, satisfied by `ScopedThreadArtifactCollisionChecker` alongside the `NativeBindingResolver` it already satisfies (`artifactcollision.go:205`). Obtained by type assertion on `Couch.Artifacts`, the pattern `actionableinventory.go:155` and `resume.go:192` already use.
  - **Why it takes candidates rather than returning the whole set:** the session-name index is **per repo scope** — `artifactpath.Resolve` puts it at `<dataDir>/repos/<RepoScope>/session-names.jsonl` (`paths.go:353,500`), and `PairSession` (`artifactcollision.go:130-167`) reads exactly one scope's index. A no-argument whole-set method would need a `<dataDir>/repos/*` enumeration the checker does not have and this plan should not add. Taking candidates mirrors the existing `PairSession(address)` / `ResolveEstablished(scope, tag, agent)` shape (`ARCH-DRY`), and the caller already holds the record set. Each candidate carries the resume proof its caller resolved (`agent`, `native id`), so the observation is COMPLETE at every layer — see the M3 Revisions entry for why patching it on afterwards was wrong.
...
**Architecture and operating envelope (M3): NOT unchanged from `#167`, and an earlier draft of this line wrongly said it was.** The pure selection is still O(n) with no fleet scan, prompt or goroutine fan-out, and a resume refusal still stops startup without creating a fallback actor. But since M2 the actionable snapshot spawns two `list-sessions` runs plus one `action list-clients` per non-exited pair session **on the host** whenever a detach candidate exists, each bounded by `zellijQueryTimeout` (5 s). M3 makes a detach candidate the *normal* case at startup, so that cost moved onto the path between typing `couch` and seeing anything — worst case (2+N)×5 s if zellij is wedged. The mitigation is the timeout, not the candidate filter: the filter bounds whether the snapshot runs, never its N. Measured at Task 12 Step 5 and recorded in the issue Log.
...
- [x] **Step 3b: PIN the M2 physicalization with an alias-path test.** (This step originally read "make detached rows carry a physical `WorkingPath`" — the M2 boundary review had already asked for that change and it shipped in `fac153c9`, so M3's contribution is the proof, not the fix. Corrected after the M3 review pointed out the record credited M3 with M2's work.) `ActionableThreadInventoryContext` re-resolves the path to physical **only** inside the parked-candidate loop (`actionableinventory.go:169-173`); detached rows would keep the stored path. Since the selector compares paths by exact string, parked and detached rows at one alias path would compare asymmetrically — one matching, one not — which is a selection bug that only shows up on a symlinked tree. Extend the physicalization to detached candidates. Step 1's alias-path cross is what proves it.
