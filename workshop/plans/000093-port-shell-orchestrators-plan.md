...
## M5 — launcher / session lifecycle → extracted to #99

M5 (the `bin/pair-shell` launcher, ~2287 lines) was carved out into its own
ticket **#99** on 2026-07-02 — the surface (~900 lines of new IO orchestration + a
new `launcher.Runtime` effect seam + the trickiest lifecycle logic in the tree,
P0) is categorically larger than the M1–M4 leaf ports and warrants its own
estimate + isolated actuals. The detailed design + phased L1–L4 plan live in
`workshop/plans/000099-port-launcher-to-go-plan.md`. **#93 stays open** until #99
lands (its Done-when includes a Go owner for the launcher).
