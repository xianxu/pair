...
- **M4b — skeleton (structure):** the 🤖[] fulfill/punt + **accept/reject** (parley §5) conversation + a **default editing posture** + **ship** (`docflow ship`) — completes the thin full cycle. The smallest set that makes the loop *usable* end-to-end. *Pair side implemented: Alt+c collaboration toggle; Alt+a/Alt+r/Alt+q review shortcuts; Copy Edit fulfill-or-punt poke posture; `:PairReviewShip` agent-owned ship request.*
...
- **`review.poke_bodies`** — pure functions building the agent-facing poke bodies: `agent_applied(applied, dropped, file)` → `"applied N edit(s) (M dropped) to <abs> — commit the agent round"`, `human_finished(file)` → `"finished my edits to <abs> — please review in Copy Edit posture; resolve 🤖[] comments as edits when possible, or punt explicitly when not"`. PURE (string-only), colocated unit test. **DRY rationale:** the round→poke wording is the protocol's commit signal (review-protocol.md seam #3); one source so the nvim and any test assert the same strings. `apply` already returns `(enriched, dropped)`, so `record.embed_in_body` (the body in the landed-artifact) is unchanged — the same encoder, just authored into a file the agent commits instead of committed inline.
...
- **`review.human_round`** — saves the human's edits and signals via `human_finished` instead of `docflow.round('human', …)`. The agent commits the saved diff (`docflow round --side human`; no body/artifact — a human round carries no records). The poke is issued by `nvim/review.lua`'s `finish_human_turn` (the UI layer), which replaced the temporary `/xx-fix` body with the structured `human_finished` signal. **Layering note:** the *agent* poke fires from the handoff watcher registered in `init.lua` (the orchestrator) while the *human* poke fires from `finish_human_turn` in `review.lua` (the UI) — a deliberate asymmetry (each poke fires where its trigger lives), not an accident to "consolidate."
...
- [x] **Step 1: failing test** — a driven `human_round` asserts the buffer is saved and **no `round --side human`** is invoked; and `finish_human_turn` pokes the `human_finished` body (not the temporary `/xx-fix …`).
- [x] **Step 2: run → fail.**
- [x] **Step 3: implement** — `human_round`: keep `save(buf)`, drop `check(docflow.round('human', …))`. In `nvim/review.lua` `finish_human_turn`, swap `poke.send(REVIEW_TRIGGER .. ' ' .. abs)` for `poke.send(poke_bodies.human_finished(file))`.
- [x] **Step 4: run → pass.**
...
- **`REVIEW_TRIGGER` retirement** — resolved in the pair pane: the `/xx-fix` poke is gone; the agent must recognize review-mode from the `human_finished`/`agent_applied` signals.
...
- **For the live smoke / #000121:** the real SKILL must (a) create `review/<slug>` on
  ...
  agent round verbatim, (c) commit the human round on the `human_finished` poke.
...
### 2026-06-21 — M4b default posture + ship seam

Filled the remaining pair-side skeleton seams without adding the M4c mode UI. The
review-open and Alt+Return pokes now name **Copy Edit** as the default posture and instruct
the agent to resolve `🤖[]` human comments as edits when possible, or punt explicitly when
not. Added `:PairReviewShip` as the pair-side ship request: it pokes the agent to run
