...
- `Frame` is **Strict-only and takes no `Mode`** (shipped; see Revisions Delta 1). A Lenient CSI arm would have had no production consumer, since `termcmd` reaches the package through `TerminatorScan`/`OSCEnd`. `Mode` lives on `OSCEnd` alone. Still live guidance: **restoring a `buf[1]` dispatch inside `csiEnd` is not an acceptable simplification** — SS3 goes through it.
