...
- **BR-16 rule as delivered (BR-18):** every per-agent dispatch reachable from `AgentInventory()` is probed: `TestProviderContractFor` and `TestAppendOnlyProviderConformance` carry qoder rows; `TestEveryAgentDispatchParity` (new) ranges scanner → ProviderContractFor → ObserveStableArtifact → ValidateDelta → NativeEventsFromRecords for every JSONL agent; `provider_live_fake_test.go` adds qoder to the live-vs-fake comparison and `validateLiveRecords`; `native_large_record_test.go` and `scan_fuzz_test.go` add qoder rows; `TestQuerySessionCatalogLossProofClassCoversEveryAgentWithoutBodyReads` adds a qoder row. The two incremental switches (`ValidateTargetWork`, `AdvanceTargetValidation`) gain fail-closed `default:` arms that error on unknown agents instead of silently skipping.
...
- **BR-19 (Important, untrusted-inputarsed-without-bounds):** `claudeFamilySpec.acceptsMillis` bounds the epoch-millis path to 2000-01-01..9999-12-31; out-of-range values dispute the record visibly. `TestQoderMillisTimestampBounds` pins the window.
