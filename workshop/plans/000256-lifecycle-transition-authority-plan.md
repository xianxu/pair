...
M2 closes here. The executable findings were all disposed as addressed and
mutation-verified by the reviewer; what remained was BR-34, and it is worth
naming exactly: **the rule "sweep every home of a changed referent" was written
down three times and failed to land seven times across four rounds — twice in the
very commit that wrote it.**

The reason is now clear. #256 retired **two** referents, not one: the park
receipt as resume authority, and `DecideResume`'s occupancy refusal (deleted in
M1). Every sweep grepped the first and never the second, so six sites survived,
including two production doc comments, the `ThreadParked` declaration I wrote
*this round* claiming two producers where the code says four, and the atlas
paragraph this window had just edited.

**So the rule stops being prose.** `TestRetiredClaimsAreNotRestatedAsCurrent`
carries the retired claims as data and greps the referent's homes — production
Go, the atlas, the README — failing when one comes back. Its scope is stated in
the test and is deliberately modest: it catches the RECURRENCE of a claim this
issue retired, not novel staleness. That is the failure mode that actually
happened, seven times.

**`TestIssue256PlanTablesMatchTheTree` gained its other direction.** Rows→tree
catches a row that lies; tree→rows catches a symbol with no row, which is the
half of C2 that let four M2 symbols go unlisted and then let a fifth
(`PreparedAgentSwitch.state`) go unlisted in the commit that added the check.

Minor, all fixed: the warm-failure assertion kept the pre-M2 verdict alive as a
dead arm that could never fire (an assertion wide enough to hold both the old and
the new answer pins neither); `presence` no longer shadows the live-proof
`observed` map.

---

## M3 re-scope (operator-approved 2026-09-17)

M2 absorbed part of M3, so M3 is re-cut before it starts — the same move Task 4
got at M2's opening. Each premise below was checked against the tree, not assumed.

| Task | Change | Why |
|---|---|---|
| **8** — archive consumes the classification | **narrowed** | Step 3 (`AllThreadStates`) landed in M2. The test HARNESS landed too: `couchtty/switch_agreement_test.go` already derives offered⇒permitted from `AllThreadStates × AllThreadReasons`. Task 8 is now `ArchivableState` + its consumers + **extending that file**, not a parallel `archive_agreement_test.go`. One table over all three actions is what the boundary review asked for. |
| **8a** — resume | **mostly absorbed** | Marked LANDED IN M1, but M2 moved `DecideResume`'s cold authority to the ledger. What remains is a resume row in the same agreement table. |
| **8b** — store keeps a record-only integrity guard | unchanged | `archivableRecord` and `occupiedIncarnation` are both still there; verified. |
| **9** — preserve Unknown in `ObserveRecordedProcesses` | unchanged | Still `if …Exists(pid) != Live { continue }`; verified. |
| **10** — archive confirms before stopping a live agent | unchanged | Step 0 (what `Quiesce` reaps) still open, still gates the `#274` dependency. |
| **11** — close the arbitrary-mutation door | unchanged | Three production callers outside the store remain; verified. |
| **12** — atlas and lessons | **narrowed** | M2 wrote most of the atlas material. `workshop/lessons.md` and the close remain. |

**Two items ADDED to M3**, both generated by M2's reviews rather than by the
original design:

- **A fake/production refusal-shape conformance table.** 3rd in family
  `production-seam-only-tested-through-fake`. M2 fixed the one instance that bit
  (the fake's absent-binding error did not wrap its sentinel, so `errors.Is` could
  never see it) and nothing prevents the next. The reviewer was explicit that this
  belongs in M3's plan rather than another per-site fix.
- **Two review rules as checked steps**, both violated in M2 and both caught only
  because the reviewer mutated my tests: *a guard's test must discriminate that
  guard's own exit* (by code or message, never a bare `err != nil`), and *a test
  named for a production entry point must invoke it*.

**Carried into Task 8 deliberately**, from the reviewer's own note:
`PrepareAgentSwitch` needed a second, record-level guard AFTER consuming the
classification, because `live` does not imply "there is bookkeeping to act on".
Archive will have the same gap in the other direction — expect it rather than
discover it.

**Known gap M3 must not lose:** the cold-side ledger read runs per
record-without-a-live-session on every refresh. `TestWarmRowsAskNoLedgerQuestion`
bounds the warm side; nothing bounds the cold side, and the 0.62 ms figure is 6
records against fakes.
