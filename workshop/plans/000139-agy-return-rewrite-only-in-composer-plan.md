...
- [x] **Step 1: Write failing fixture inventory/schema tests**
...
- [x] **Step 3: Extend the capture helper into live conformance**
...
- [x] **Step 4: Capture literal real-harness bytes**
...
- [x] **Step 5: Replay through the production seam**
...
- [x] **Step 6: Verify every supported harness exactly**
...
### 2026-08-19T11:55:00-07:00 — Live conformance is behavioral; add Codex recognizer restoration

Executing Task 6 Step 4 against the three installed harnesses produced evidence
that invalidates two of Task 6's assumptions. Both corrections are recorded here
before further implementation.

#### Finding 1: the Codex recognizer no longer matches installed Codex

`codexComposerActive` requires a `57,57,57` background band on a row adjacent to
the cursor. That signal is inherited from #137's retired `codexComposerBG =
"2;57;57;57"`. Codex `0.147.0` paints no background at all: its composer is a
`›` at column 0 with the cursor at column 2, and every row carries the default
background. The positive gate therefore never fires, plain Return always takes
the bare-CR path, and Enter cannot insert a newline in the Codex composer.

This break predates #139 — it is #137 surface that Task 6's live conformance is
the first thing to observe — but #139 now owns the Codex profile and Task 6
Step 6 requires the Codex live check to pass. Closing #139 with a permanently
non-firing Codex gate would ship the exact defect the issue exists to prevent.
Task 6A below restores recognition from literal evidence, in the same
capture-then-define order Task 2A established for Muse (ARCH-PURPOSE).

Codex `0.147.0` additionally blocks startup on an update interstitial. Its argv
gains the documented `-c check_for_update_on_startup=false`, the counterpart of
Muse's `MUSE_NO_AUTO_UPDATE=1`, so startup capture is reproducible rather than
dependent on whether a newer release exists.

#### Finding 2: byte-exact live comparison asserts the wrong invariant

Step 5 had `TestHarnessTTYLiveConformance` assert `bytes.Equal` between the live
capture and the checked-in fixture. Real captures embed per-account,
per-machine, per-moment content: Agy paints the signed-in account address and
cwd, Codex paints a weekly rate-limit banner and a rotating tip, Muse paints the
model name. Agy also self-updates — it moved `1.1.13 -> 1.1.15` between two runs
minutes apart, invalidating a just-captured fixture directory. Muse is
deterministic run to run (three consecutive captures share one digest) yet no
longer matches a two-day-old fixture at the same version.

Byte identity is therefore unachievable for the live path, and failing on it
reports drift that has nothing to do with Return correctness. The invariant the
live path actually defends is behavioral: the production recognizer still fires
on the real installed harness's composer, and the resulting Return decision is
the expected positive-composer decision. Byte-level exactness moves entirely to
the fixture path, where frozen bytes make it achievable by construction — that
is Step 5's every-split replay, which remains unchanged and is the stronger of
the two checks. The live path keeps logging version, argv, and digest, keeps
printing the recapture destination, and reports version drift as information
rather than failure. A behavioral live check would still have caught Finding 1.

#### Task 6 step deltas

- Step 3 and Step 5: the live check asserts recognition plus expected Return
  decision, not `bytes.Equal`. Version mismatch against fixture metadata logs a
  recapture hint instead of failing.
- Step 4: capture Agy and Codex fixtures and recapture Muse at current content,
  each trimmed to the first recognizing prefix by the shared helper rather than
  a per-harness marker function.
- Step 6: unchanged in intent — all three installed harnesses must pass — but
  the Codex pass now depends on Task 6A.

### Task 6A: Restore Codex composer recognition from literal evidence

**Files:**
- Modify: `cmd/internal/wrapcmd/composer_recognizers.go`
- Modify: `cmd/internal/wrapcmd/composer_recognizers_test.go`
- Modify: `cmd/internal/wrapcmd/harness_tty_live_test.go`
- Create: `cmd/internal/wrapcmd/testdata/tty/codex/<version>/composer.raw`
- Create: `cmd/internal/wrapcmd/testdata/tty/codex/<version>/overlay.raw`

- [x] **Step 1: Capture literal Codex composer and overlay bytes**

Capture the composer through the live path. Capture at least one genuine
overlay where plain Return must confirm rather than insert a newline; the
update interstitial (`1. Update now / 2. Skip / 3. Skip until next version /
Press enter to continue`) is already reproducible by omitting the update-check
override. Drive further overlays only through the bounded helper, never by
hand-authoring bytes. Both raw files carry digests in one `metadata.json`.

- [x] **Step 2: RED the recognizer against that evidence**

Add geometry cases derived from the captured snapshots: the real composer must
recognize; every captured overlay must not. Keep the existing frozen Codex rows
that still describe reachable states and delete only rows whose premise — the
`57,57,57` band — no longer exists in any installed Codex. Removing a frozen row
requires naming it in the issue Log.

- [x] **Step 3: Implement the minimal predicate**

Define recognition from the structure the evidence actually shows, not from one
sample's coordinates: a prompt glyph at the composer column with the cursor on
the same row beyond it. Prefer reusing the shared cell/geometry helpers over a
third bespoke scan (ARCH-DRY). The predicate stays pure over `terminalSnapshot`
and must return false for every captured overlay and for unknown/invalid
snapshots.
