...
**1. Deprioritise rather than cap.** Launch session work at a lower QoS tier so
builds absorb idle capacity but yield to interactive work. Self-balancing in
principle: a lone session still gets all 12 cores, where a static cap would
leave 9 idle. **Its supporting measurement was withdrawn (see above) — treat
this as the hypothesis to test first, not the preferred answer.**
...
- [ ] **Rebuild the experiment first.** Drive it with N concurrent real
      `go test ./...` runs across repos — never synthetic CPU burn — and measure
      `zellij action` round-trip (the metric that actually moved 8×), not just
      wake-up delay (which moved 2×, and 0× under the invalid probe).
- [ ] Then compare `-c background` vs `-c utility` vs a parallelism cap on that
      harness: interactive latency **and** build wall-clock, so the IO throttle
      cost is visible.
