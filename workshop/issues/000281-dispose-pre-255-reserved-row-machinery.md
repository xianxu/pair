...
| `hostty.Reservation.ReserveAndPaint` / `Paint` / `Release`, `hostty.SetRegion` | `cmd/probes/couchnestedrows` only. Production uses `Reservation` for `ChildRows` arithmetic (`couchtty/console.go:1019`). |
