...
- `make test` run from *inside* a live pair session needs the hosted session's retention-owner env scrubbed, or two targets fail as phantom regressions: `pair session-log append` refuses the test's `PAIR_LOG_PATH` ("retention: log path does not match selected owner") because the leaked `PAIR_DATA_DIR`/`PAIR_TAG` select the live owner, and scrubbing `PAIR_TAG` alone leaves `PAIR_RETENTION_PROTOCOL` armed with no owner ("PAIR_DATA_DIR and PAIR_TAG are required"). The whole group goes together: `env -u PAIR_DATA_DIR -u PAIR_TAG -u PAIR_RETENTION_PROTOCOL -u PAIR_RETENTION_BACKGROUND -u PAIR_RETENTION_START_ID make test`. Scrubbing *all* `PAIR_*` overshoots — `tests/workbench-route-nvim-test.sh` needs `PAIR_WORKBENCH_LAYOUT_PATH`. Prove an env-shaped failure by re-running the single target scrubbed before treating it as a code regression. (#266, 2026-09-16)

- The same run also needs a **real** `TMPDIR`. Claude Code's scratchpad exports `TMPDIR=/tmp/claude-501`, and `/tmp` is a symlink to `/private/tmp` on macOS: `tests/changelog-open-test.sh` then fails `pair-changelog-open: viewer: process target is outside selected owner directory`, because `storagegc.validateProcessTarget` compares the target against the owner directory **lexically** (`filepath.Rel`) while the two sides disagree about symlink resolution. Point `TMPDIR` at a non-symlinked directory and it passes. Reproduced identically at `f4969cfe` (before this issue's first commit), so it is environmental, not a regression — but the lexical comparison is a genuine latent weakness for any real `PAIR_DATA_DIR` reached through a symlink. (#266, 2026-09-16)

### 2026-09-16 — #266 close boundary review (4 rounds)

- A deliberate **non**-behavior is still a behavior and needs a named test. The in-paste Alt+Enter interception was added, removed, restored and removed again across four commits and no test was ever red, because "the translator does not do this" was only ever the absence of code. Absence of code is not regression evidence. (BR-12)
- When you delete a branch, **triage its test rows** instead of deleting them with it. The revert took three rows, and one pinned the surviving post-paste path — so the contract the issue existed to defend ended up pinned on its negative half alone. (BR-16)
- A check that **retires an acknowledged gap must test the exact property the gap names**. An oracle that reads one boolean where the property has three components will retire a gap that still exists; conjoin every component the data model can express, add a field for what it can't, and say in that field's doc which part is honor-system. (BR-15, BR-19)
- A claim about how an external harness **reacts** to bytes we emit is not replayable: a capture proves what our side emitted and stops there. Drive it or record it in an expiring ledger — never assert it in a comment. Four such claims were standing here, one per harness. (BR-14)
- When a finding names a site, **fix the class**: enumerate with a grep before calling it addressed. BR-17 came back `not-addressed` because two of three sites were fixed and the third was never looked for.
- A comment that names a path, a version, a registry entry or a symbol is a claim the tree can check — so check it mechanically. `TestTTYFixtureReferencesResolve` and `TestDocCommentsNameTheirSubject` retire two families that had each recurred four times. Generalize carefully: "a doc comment must open with its declaration's name" flagged 17 innocent prose comments, while "a *symbol-shaped* first word must resolve and belong" found exactly the two real ones. (BR-13, BR-18)
- A frozen capture must not embed the **capture machine's filesystem**, or a recapture elsewhere differs for reasons unrelated to the drift it exists to detect. Mechanize it; if a harness genuinely cannot comply, measure why before writing the acknowledgment. (BR-21)
- Any artifact family that replays on every `go test` needs a written **retention rule** and a measured cost. One three-screen fixture set was ~45% of its suite's runtime. (BR-22, `ARCH-FUNERAL`)
