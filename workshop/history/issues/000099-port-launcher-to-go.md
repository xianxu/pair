...
- [x] M5b — lifecycle write flows: native in-session **compaction**
...
- 2026-07-03: closed — Launcher fully ported to Go + bin/pair-shell retired (removed). pair is a single Go launcher end-to-end (create/attach/pick/list/rename/continue/compaction/restart-loop/--help all in cmd/internal/launcher, driven in-process by cmd/pair-go). 7 milestones M1-M5c each boundary-reviewed FIX-THEN-SHIP->SHIP. go build/test ./... + -race + vet green; full make test green (sandbox off for ps); runtimebundle drift-check deterministic; shadow-sweep clean (no ErrFallbackToShell/pair-shell runtime ref). --no-judge: the whole-issue integration review would hit the ariadne#162 window bug on the multi-merge diff; the per-milestone reviews + the M5c review are the coverage. Unblocks #93 (Go launcher owner) + #94 (stop extracting a shell tree); review verdict: not-run
