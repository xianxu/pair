...
This guide outlines how to bring up a new agent harness CLI (e.g., `muse`) and achieve parity with existing agents (`claude`, `codex`, `agy`, `muse`, `qoder`).
...
- **For `qoder`:** Qoder renders both families too — a permission picker ("Permission Required" over a question, with body rows "Reject and type something" / "…for future sessions") and its question picker (AskUserQuestion: an "Asking User" header over a ruled card, closed by a keybinding footer `↑↓navigate·Enterselect·Esccancel`). Markers are in `qoderPickerMarkers`, verbatim as the *stripped* text sees them: Qoder paints body rows word-by-word at absolute columns, so they glue (`Rejectandtypesomething`, `Enterselect`), while the header emits real space bytes. A marker must not be ordinary English prose: the capture's "for future sessions" was a marker in the first cut and was dropped, because a word-by-word paint glues it into `forfuturesessions` wherever the words appear, arming `pickerActive` off agent output and turning the user's next composer Enter into a submit (pinned negative in `TestOverlayDetectorByAgent`). The one deliberate exemption is the title-case header "Permission Required": it is ordinary English, but it titles *every* permission picker while the body strings pin only the one captured question, so dropping the header would leave a picker for a different tool unrecognized. The exemption's boundary is pinned (`qoder spaced prose about permissions does not open overlay`): the topic in other words, in other case, or not contiguous stays closed, and the exposure is bounded by `pickerActive` being consumed by exactly one Enter. The permission family thus stays covered three ways — the generic header plus the two glued body strings. `detectQoderOverlayOpen` scans both the stripped visible tail **and** the raw rolling buffer: per-chunk stripping keeps a chunk-split escape verbatim, so a boundary inside a marker's style run (replaying `selection.raw` split at 6426 cut `\x1b[23m` between `Enter` and `select`) can sever the marker in the tail, which the byte-contiguous rolling window cannot; that window is proxy-owned (`overlayRawTail`) so `emitPlainCR` clears it when the confirming Enter consumes `pickerActive`, or the consumed bytes re-arm the flag.
...
- Add the agent-specific binding to `resumeToken`, place it through
  ...
  `resume <id>` subcommand; Claude and Qoder share the `--resume`/`-r` family
  (space, `=` and glued `-r<id>` forms), and Agy uses `--conversation <id>`. One
  `resumeform` table owns every agent's spellings for extraction, stripping and
  fresh-arg validation alike (pair#300 BR-15) — add a row there, not a site.
