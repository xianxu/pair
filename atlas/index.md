...
- [Session identity and storage](session-identity.md) — repo-scoped tags, the `📁` public zellij name scheme and its byte budget, per-tag ledgers, and legacy flat-data recovery.
...
- [couch](couch.md) — the session supervisor (`cmd/couch`): one leased namespace, composite durable threads, recoverable pre-exec starts, normalized fleet-policy admission, and tty routing.
