...
- `doctor/README.md` — `pair-doctor`: read the adaptation flight recorder to diagnose harness integration drift (see the bring-up guide §3 for the signal registry). Also packaged as the unregistered `doctor/SKILL.md` skill (link `doctor/` into `.claude/skills/` to activate).
