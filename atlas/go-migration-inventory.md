...
| `zellij/layouts/main-{2,3}.kdl` | zellij native assets | native launcher via `zellij --new-session-with-layout` | Layout 2 defines agent/draft and is the unrecorded default; layout 3 adds the filler plus floating user terminal. Both expand Pair env at pane start and preserve the shared draft ladder. | Calls `pair wrap`; calls `nvim -u "$PAIR_HOME/nvim/init.lua"`; layout 3 calls `pair term`; writes `pane-<tag>-<agent>.json` and the draft nvim pid file. Both assets are required by source and embedded-runtime root validation. | native assets, packaged adjacent/embedded | P0 |
