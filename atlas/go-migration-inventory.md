...
| `bin/pair-slug` / `cmd/pair-slug` / `cmd/internal/slugcmd` | Go binary plus shared runner | `pair-wrap` turn-end hook (now `pair slug`), tests | Env-driven, no stdin; resolves native transcript, proposes slug; exits 0 on most failures. | Consumes `$PAIR_AGENT_CONFIG_PATH`, `$PAIR_SLUG_PROPOSED_PATH`, and `$PAIR_ADAPT_LOG_PATH`; reads native transcripts/git branch; optional `PAIR_SLUG_*`, `OPENAI_API_KEY`. | implemented `pair slug` route (#92, buffered `slugcmd.Run`); `bin/pair-slug` retained as thin shim | P1 |
| `bin/pair-context` / `cmd/pair-context` / `cmd/internal/contextcmd` | Go binary plus shared runner | `cmd/internal/titlepoller` (in-process, #93 M1); development-only `pair-go context` | `pair-context <tag> <agent>` and `pair-go context <tag> <agent>` print the same humanized token count or nothing; tolerant exit 0 on failure. Exposes `TranscriptPath` for the shared transcript resolution. | Reads exact pane/config bindings plus native transcripts. | implemented `pair context` route; the title poller now calls `contextcmd.Run`/`TranscriptPath` **in-process** (#93 M1, no subprocess); `bin/pair-context` retained as thin shim | P1 |
...
| `pair session-inventory` / `cmd/internal/sessioninventory`; `pair session-watch` / `cmd/internal/sessionwatch` | canonical native forest/correlation CLI plus stateful establishment watcher | operators/agents; launcher create and agent-only restart | Inventory emits stable human/schema-v1 forests, bindings, ambiguities, diagnostics, and redacted conformance. Watcher monitors one typed launch ordinal and persists only a unique completed causal round; no first/newest selection and no Claude exception. | One injected inventory runtime owns native filesystem/SQLite/process/open-file reads; `sessionledger.LedgerStore` owns locked append/fsync; config is a post-binding cache. | #155 M2 canonical boundary; final #155 migration moves remaining point consumers onto it | P1 |
