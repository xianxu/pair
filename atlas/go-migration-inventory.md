...
| `zellij/layouts/main-{2,3}.kdl` | zellij native assets | native launcher via `zellij --new-session-with-layout` | Layout 2 defines agent/draft and is the unrecorded default; layout 3 adds the filler plus floating user terminal. Both consume exact Pair bindings at pane start and preserve the shared draft ladder. | Calls `pair wrap`; calls `nvim -u "$PAIR_HOME/nvim/init.lua"`; layout 3 calls `pair term`; writes exact `$PAIR_AGENT_PANE_PATH` and `$PAIR_NVIM_DRAFT_PID_PATH`. Both assets are required by source and embedded-runtime root validation. | native assets, packaged adjacent/embedded | P0 |
...
| `bin/pair-wrap` / `cmd/pair-wrap` / `cmd/internal/wrapcmd` | Go binary plus shared runner | zellij agent pane | `pair-wrap [--scrollback-log PATH] agent [args...]`; transparent PTY proxy; long-running; failure in diagnostics is swallowed. | Consumes exact `$PAIR_AGENT_OUTPUT_PATH`, `$PAIR_AGENT_PID_PATH`, `$PAIR_SCROLLBACK_RAW_PATH`, `$PAIR_SCROLLBACK_EVENTS_PATH`, and image-capture bindings; may invoke `pair slug`. | implemented `pair wrap` route (#96, streaming seam — real stdio via `wrapcmd.Run`); `cmd/pair-wrap` is a thin shim over `cmd/internal/wrapcmd`; KDL still execs `pair-wrap` by PATH | P0 |
| `bin/pair-slug` / `cmd/pair-slug` / `cmd/internal/slugcmd` | Go binary plus shared runner | `pair-wrap` turn-end hook (now `pair slug`), tests | Env-driven, no stdin; proposes a slug only after `QuerySession` returns an established scanner-authorized root; exits 0 on most failures. | Consumes `$PAIR_SLUG_PROPOSED_PATH` and `$PAIR_ADAPT_LOG_PATH`; reads the bounded inventory-owned text-event window plus git branch; `PAIR_SLUG_*` model settings and `OPENAI_API_KEY` remain. | implemented `pair slug` route (#92); #155 removed config/lsof/native-path resolution, raw transcript access, native parsers, and the old transcript package | P1 |
| `bin/pair-context` / `cmd/pair-context` / `cmd/internal/contextcmd` | Go binary plus shared runner | `cmd/internal/titlepoller` (in-process, #93 M1); development-only `pair-go context` | `pair-context <tag> <agent>` and `pair-go context <tag> <agent>` print the same humanized token count or nothing. They exit 0, printing nothing, for an unbound or provisional owner, and exit 2 with a stderr reason when `PAIR_SCOPE_KEY` is missing (`pair#183`). | Queries the established owner through `sessioninventory` and reads usage only from its authorized root; `ctxmeter` humanizes. | implemented `pair context` route; #155 removed pane/config/native-path transcript selection | P1 |
...
| `cmd/internal/adapt` | Go helper package | `cmd/internal/wrapcmd` (pair-wrap), `pair-slug`, tests | Pure-ish emitter helpers plus file open seam; no command. | Writes the validated `artifactpath.Paths.AdaptLog`; schema shared with shell/Lua. | internal package, reuse behind dispatcher | P1 |
| `cmd/internal/ctxmeter` | Go helper package | `pair-context`, tests | Agent-neutral token-count humanization. | No direct IO. | internal package, keep | P1 |
...
| `cmd/internal/transcript` | retired Go helper package | none | Formerly resolved native transcript paths and session ids. | Deleted. | removed #155; `sessioninventory.QuerySession` + scanner-authorized artifacts own the boundary | P1 |
| `cmd/pair-scrollback-open` / `cmd/internal/opener` | Go binary plus shared runner | zellij Alt+/ Run, nvim Alt+b jump | `pair-scrollback-open [--jump prev|next]`; opens read-only nvim viewer; singleton lock. | Resolves one `artifactpath.Paths.ScrollbackArtifacts` set; renders in-process (`scrollbackcmd`), uses zellij IPC and nvim, and writes only its exact ANSI/viewport/lock paths. | ported to Go (#93 M2) on the #78 template — pure viewport scorer in `opener`, IO behind the `Runtime` seam; **replaces** the shell script at the same PATH name (zellij invokes by name → no shim); `nvim/scrollback.lua` stays native | P1 |
| `nvim/scrollback.lua` | Neovim native asset | `cmd/pair-scrollback-open` | Loaded by `nvim -u ... <ansi>`; interactive read-only viewer; refreshes backing render. | Consumes the exact `$PAIR_SCROLLBACK_*_PATH` companion bindings; may call `pair-scrollback-render`; writes only `$PAIR_SCROLLBACK_PENDING_PATH`. | native-asset, adjacent/embedded | P0 |
| `cmd/pair-changelog-open` / `cmd/internal/opener` | Go binary plus shared runner | zellij Alt+l Run | Opens changelog viewer and starts detached render/distill singleton. | Resolves one `artifactpath.Paths.ChangelogArtifacts` set, passes exact log/anchor/cleaned/lock/status/ready paths to the detached build, and launches the nvim watcher with exact bindings. | ported to Go (#93 M2) — shared `opener` package (session keying + detached distiller), IO behind the seam; **replaces** the shell script at the same PATH name (no shim); Go `SysProcAttr.Setsid` replaces setsid/perl | P1 |
| `nvim/changelog.lua` | Neovim native asset | `cmd/pair-changelog-open` | Loaded by `nvim -u ... <log>`; read-only watcher/spinner. | Consumes exact `$PAIR_CHANGELOG_PATH` and `$PAIR_CHANGELOG_READY_PATH` bindings. | native-asset, adjacent/embedded | P1 |
| `bin/pair-title` / `cmd/pair-title` / `cmd/internal/titlepoller` | Go binary plus shared runner | launcher `SpawnTitlePoller` | `pair-title <tag> <agent>`; long-running 60s poller (frame meter + cmux heat-ramp). | Uses `Paths.TitlePID`, `Paths.PaneGlob`/`AgentFromPane`, and the cmux ownership store; calls zellij/cmux/ps + in-process `contextcmd` for the count. | ported to Go (#93 M1) on the #78 sessionwatch template — pure decisions in `titlepoller`, IO behind the `Runtime` seam; the `.sh` re-exec shim was retired in #94 M2 (the launcher spawns `bin/pair-title` directly) | P1 |
| `pair session-inventory` / `cmd/internal/sessioninventory`; `pair session-watch` / `cmd/internal/sessionwatch` | canonical native forest/correlation CLI plus stateful establishment watcher | operators/agents and every native-session consumer | Inventory emits stable human/schema-v1 forests, bindings, ambiguities, diagnostics, redacted conformance, and an internal established-root activity projection. Watcher monitors one typed launch ordinal and persists only a unique completed causal round; no first/newest selection and no Claude exception. | One injected inventory runtime owns native filesystem/SQLite/process/open-file reads; `sessionledger.LedgerStore` owns locked append/fsync; config is launch-argument compatibility only. | #155 canonical boundary; query, usage, activity, and shadow contracts prevent independent consumer authority | P1 |
| `bin/lib/adapt-log.sh` | sourced shell helper | remaining shell emitters | `adapt_log comp agent aspect signal outcome [detail]`; no-op if no exact path binding or jq. | Appends JSONL to `$PAIR_ADAPT_LOG_PATH`. | keep until remaining shell emitters move; schema stays DRY with Go/Lua emitters | P1 |
...
| `nvim/doctor.lua` | Lua helper | `:PairDoctor` in nvim | Builds agent instruction payload (drift + the #208 perf capture: headline, pid join, rolling row). | Reads `PAIR_HOME`; consumes the draft buffer as the note; sends text through draft/agent flow. | native-asset | P2 |
| `bin/pair-notify` | Bash notification helper | agent hooks/manual shell inside Pair | `pair-notify [--osc 9|777] "message"`; writes OSC to outer tty; nonzero on bad args/missing tty. | Requires `PAIR_TAG` plus exact `$PAIR_OUTER_TTY_PATH`; never reconstructs from data dir/tag. | small shell-glue; possible Go subcommand but low packaging impact | P2 |
...
| `cmd/pair-review-target` / `cmd/internal/reviewcmd` | Go binary plus shared runner | review readiness/open/tests | Writes session-stamped JSON target metadata. | Consumes exact `$PAIR_REVIEW_TARGET_PATH` and `$PAIR_AGENT_CONFIG_PATH`; Codex fallback uses `cmd/internal/codexsid` (ps/lsof). | ported to Go (#93 M3); session resolution reuses `transcript`-style config read + the extracted `codexsid` walk; replaces the shell script (no shim) | P2 |
| `nvim/init.lua` | Neovim native asset | zellij draft pane | Main draft UI and Pair key handling. | Consumes exact `PAIR_*_PATH` bindings, including image-done and changelog-ready companions; shell-outs to zellij and Pair helpers without reconstructing selected artifact names. | native-asset; do not port, but audit shell-outs during #77/#78 | P0 |
...
- `zellij/layouts/main-{2,3}.kdl` launch `pair-wrap` by PATH and `nvim -u
...
- #93 M3 ported the three review-start orchestrators to `cmd/pair-review-target`
  ...
  `cmd/internal/codexsid` (the ps/lsof rollout-uuid walk, later retired by #155
  when all consumers moved to `sessioninventory`). The Go binaries replace the shell scripts
...
- **#104 M2 (rewrite every owned caller to `pair <sub>`)** repointed all in-repo
  ...
  - `zellij/config.kdl`: `copy_command "pair clip copy-on-select"`, and Alt+/ /
    ...
    `zellij/layouts/main-{2,3}.kdl` agent pane `exec pair wrap …`.
...
- `cmd/internal/codexsid/codexsid.go` (removed #155 — inventory query owns native identity)
- `cmd/internal/codexsid/codexsid_test.go` (removed #155)
...
- `cmd/internal/transcript/transcript.go` (removed #155 — scanner-authorized root queries replace path resolution)
- `cmd/internal/transcript/transcript_test.go` (removed #155)
