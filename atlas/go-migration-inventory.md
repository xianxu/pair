...
| `bin/pair-context` / `cmd/pair-context` / `cmd/internal/contextcmd` | Go binary plus shared runner | `cmd/internal/titlepoller` (in-process, #93 M1); development-only `pair-go context` | `pair-context <tag> <agent>` and `pair-go context <tag> <agent>` print the same humanized token count or nothing; tolerant exit 0 on failure. | Queries the established owner through `sessioninventory` and reads usage only from its authorized root; `ctxmeter` humanizes. | implemented `pair context` route; #155 removed pane/config/native-path transcript selection | P1 |
