...
| `bin/pair-notify` | Compatibility shim to `pair notify` | agent hooks/manual shell inside Pair | `pair-notify [--osc 9|777] "message"`; bounded best-effort broker send; invalid args fail. | Requires exact `$PAIR_PAIR_WRAP_PID_PATH`; wrapper serializes notification through Zellij. | Go command with shell alias | complete |
