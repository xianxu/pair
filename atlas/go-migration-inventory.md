...
| `bin/pair-slug` / `cmd/pair-slug` / `cmd/internal/slugcmd` | Go binary plus shared runner | `pair-wrap` turn-end hook (now `pair slug`), tests | Env-driven, no stdin; proposes a slug only after `QuerySession` returns an established scanner-authorized root; exits 0 on most failures. | Consumes `$PAIR_SLUG_PROPOSED_PATH` and `$PAIR_ADAPT_LOG_PATH`; reads the authorized transcript plus git branch; optional exact `PAIR_SLUG_TRANSCRIPT`, `PAIR_SLUG_*`, `OPENAI_API_KEY`. | implemented `pair slug` route (#92); #155 removed config/lsof/native-path resolution and the old transcript package | P1 |
