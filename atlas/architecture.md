...
`keyhelp` **derives wording** rather than restating it, from four sources because no single one covers the surface: `desc = 'pair: …'` on each `vim.keymap.set` (the draft keys — already good prose); `workbenchshortcut.GlobalBinding.Help` (the global chords, which reach nvim via the *generated* `workbench_actions.lua`, so parsing `init.lua` cannot see them); `workbenchshortcut.RoleBinding.Help` (pane-local terminal chords — their nvim descs describe the deliberate draft **no-op**, so deriving from nvim would document a working feature by what it does where it does nothing); and the two zellij `Run` binds, the only rows whose text is authored in the catalog because KDL has no description field.
