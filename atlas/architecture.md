...
2. **Two consumers** of the captured path:
   ...
   - **`bin/pair-wrap`** (Go, `cmd/pair-wrap`). Transparent PTY proxy. The zellij agent pane runs `pair-wrap $PAIR_AGENT $PAIR_AGENT_ARGS` instead of the agent directly (wired in `zellij/layouts/main.kdl`). The wrapper allocates a fresh PTY for the agent, forwards stdin/stdout transparently with SIGWINCH propagation, and watches the agent's output stream for OSC notifications. On detection it writes OSC 9 directly to the recorded outer-TTY path — bypassing zellij.
     ...
     **Overlay-aware suspension (claude only).** The `\<CR>` remap is wrong while a blocking overlay (AskUserQuestion picker, tool-permission prompt) has focus — the overlay consumes the `\r` and the leading `\` leaks into the textarea behind it. pair-wrap watches the agent output stream for `OSC 777;notify;Claude Code;Claude needs your permission` (the unified open trigger for both overlay shapes), sets `pickerActive`, and emits a bare `\r` for the next plain Enter so the overlay confirms cleanly. The flag clears after that one Enter — the textarea-aware remap is back in effect for the next keystroke. Known edge: ESC-dismissing an overlay without Enter leaves the flag set, so the first post-dismiss Enter submits the draft instead of inserting a newline (issue #000023).
