...
Detection of attached-vs-detached uses `zellij --session NAME action list-clients`, which prints a header plus one row per connected client. Zero rows = detached. That call costs about 250 ms against a real detached session (`pair#228`, `cmd/probes/reattachcost`), and `list-sessions` carries no client information, so one snapshot routine (`ZellijSource.snapshot`) serves three forms by who needs what. The full form (`SnapshotContext`, `OSRuntime.Sessions`) asks every session; bare `pair`'s picker uses it, as do `pair rename`'s gate and the create flow's re-check after deleting a conflicting session. `SnapshotSessionsContext` asks only named sessions and is used by couch's detached proof. `LivenessContext` asks none, reporting `SessionLive`, and is used by `pair resume`, the orphan-nvim sweep, and couch's registration poll. `DecideLaunch`'s `launchShape` switch is what decides which form `runOnce` takes. Every reader of attached-versus-detached (`DecideLaunch`'s bare branch, the picker, couch's `ProjectDetachedSessions`) first applies `launcher.RequireAttachState`, which refuses a `SessionLive` row. `pair list` is a separate full scan (`OSRuntime.ListSessions`) with its own `list-clients` per session.
...
**Title poller (`pair title` + `cmd/internal/titlepoller`; #93 M1, self-exec since #104 M2) — two surfaces.** A single always-on per-tag 60s background poller, spawned on every entry so a poller a host sleep/reboot/SIGKILL killed is reliably revived. Its single-instance guard is identity-checked (`pollerArgvMatches` plus `artifactpath.Paths.TitlePID`, not a bare `kill -0`) so a recycled PID cannot suppress the respawn.

**Birth gate (`pair#287`).** The poller makes no zellij call until the agent pane's sidecar (`panebirth.Evidence`, the one declaration of that path) exists. It stats every 250 ms through `panebirth.Await`, bounded by `StartupGrace` (30 s), and exits without touching zellij if the pane never appears.
- *Why.* zellij 0.45.1 panics (`RemoveClient` unwraps `session_data`) when a connection it accepted before the first client initialized the session closes. `list-sessions` connects to every socket. The poller is spawned just before `zellij --new-session-with-layout`, so its old startup probe killed the session it was waiting for: 4 cold launches in 10 died (`probes/zellijbirthrace`).
- *Why the sidecar counts as evidence.* A pane exists only once the session has initialized, and the layout's pane command writes the sidecar first thing.
- *Freshness.* The create path clears this agent's sidecar before it spawns the session watcher, the poller or zellij, the same pattern as `RemoveReadyRecord`. Attach never clears, so a live session's pane passes the gate on the first stat.
- *One wait (`pair#288`).* `cmd/internal/panebirth` holds the evidence path and `Await`, the poll loop that all three birth waiters share: this gate, Couch's cold-resume registration, and the launcher's birth watch. A failed observation is "not yet", never birth, and does not end the wait. A grace that passes reads as `context.DeadlineExceeded`, so Couch's registration diagnosis survives its own context racing the grace.

**Launcher birth watch (`pair#288`).** A zellij server can still die at birth: any machine-wide `list-sessions` (another thread's poller, `pair list`) can land in its window. Most such clients notice and exit 1. Some hang forever on a cooked tty, having written nothing, so `LaunchSession` never returns. Couch then keeps the thread live, because its helper is alive: the astro casualty sat that way for 2 h. `launchWatched` (`launcher/birthwatch.go`) runs the create handoff under a watch goroutine. It is started just before `LaunchSession` and stopped and joined as soon as it returns.
- *Bound.* The watch waits `birthBound` (10 s) for the pane's sidecar. Measured launch-to-pane, end to end: 0.6–1.3 s idle, 1.5–2.9 s with every core saturated, 0.4–0.5 s with a cold zellij cache. 10 s is under Couch's 15 s registration deadline.
...
- *Conformance.* The fake models zellij 0.45.1's hang: the client writes 0 bytes, isn't listed, and exits on SIGTERM. Re-run `probes/zellijbirthrace launch -hammer 10ms` on every zellij version change. It counts `hung` launchers and clients left behind.

**Its launch contract is `titlepoller.SessionEnv`**: `PAIR_DATA_DIR` and `PAIR_SCOPE_KEY`, built by the positional `NewSessionEnv`. It is handed to the spawn by both create and attach, not inherited from whatever the launcher exported first. Inheritance was `pair#183`: attach's hand-copied export subset lacked the scope key, and every reattached thread silently lost its context meter. `TestSessionEnvSuppliesEveryPairVariableThePollerReads` ties the contract to the reads it observes, so a new read cannot be missed the same way. When attach cannot resolve a repo root, the scope falls back to couch's own record of *this* thread (`COUCH_THREAD_SCOPE` when `COUCH_THREAD_TAG` names the tag). Failing that, the contract carries an explicit empty key and does not let the child inherit one, because an inherited key belongs to whatever ran `pair` and could paint another session's count. No meter beats a wrong one. One consequence of the first-wins guard: a poller spawned without scope survives an upgrade until its process group dies, for example on a couch detach. It owns two title surfaces (tested in `cmd/internal/titlepoller`):

- **Per-pane context meter in the zellij FRAME (#71).** Each agent pane's frame title reads `<agent> (<count>)`, where `<count>` is the current context-window occupancy. `contextcmd` queries the exact established owner through `sessioninventory`, then reads token usage only from that scanner-authorized root; `ctxmeter` owns only agent-neutral humanization. Provisional, ambiguous, and unbound owners print nothing. A missing scope key is not one of those. No ledger record can match an empty key, so `pair context` reports it as `ExitNoScopeKey`, with a reason on stderr (`pair#183`). The status is 2, which it shares with the dispatcher's usage errors; stderr tells them apart. The pane sidecar locates the UI surface but no longer selects a transcript. The poller resolves the count in-process, filters stale pane twins by active agent, and renames the pane through the actual public zellij session name, gated on recent inventory activity. Cleanup removes the exact pane sidecar on Alt+x. Always-on (the frame exists with or without cmux). Carried through `pair rename` like `config-*`.
...
the structured agent command under `pair wrap --from-launch-env`, and the bottom pane is
...
`$PAIR_AGENT_COMMAND` carries the executable and exact argv as strict JSON. The agent pane invokes `pair wrap --from-launch-env`, which decodes the envelope directly into the child command. Empty arguments, spaces, quotes, Unicode, and shell metacharacters survive without expansion. Ordinary explicit `pair wrap <command>` ignores inherited command authority.
...
Keybindings use `clear-defaults=true` (#245). The explicit KDL bindings only
...
- Pane-local shortcuts (#116/#123): `Alt+j` focuses the agent from the draft; `Alt+k` bridges draft/right-terminal, returning from the terminal to the last focused left pane via exact `$PAIR_LAST_LEFT_PANE_PATH`; `Alt+t`/`Alt+w` create and close tabs only in the right terminal; `Alt+r` enters the terminal wrapper's rename editor (#118; the field renders in the strip since M4 took the frame off), whose pure rune editor and streaming decoder consume all edit/control bytes before the child PTY and use `rename-pane` as the sole title IO boundary; `Alt+Shift+d` in the right terminal is a native tiled split — `new-pane --direction down` on the invoking (client-focused) terminal, running the layout-3 `pair term` shell (BORDERLESS since `#199` M4 — each half's `pair term` draws its own strip in a reserved row, which carries the tab title the frame used to and is what separates the halves); `Alt+/` and `Alt+Shift+C` / `Ctrl+Alt+c` operate from the draft; agent input passes through. Every left→right jump is id-based: draft nvim calls `pair layout focus-terminal` (`layoutcmd.FocusRightTerminal`), which focuses the tiled right terminal via `focus-pane-id` — the recorded last-used half wins (exact `$PAIR_LAST_TERMINAL_PANE_PATH`, written when `Alt+k` leaves the terminal side; zellij's `is_focused` on right-side panes is stale memory while focus sits in the left stack), else a zellij-focused half, else the first. Split halves are recognized through exact `$PAIR_TERMINAL_PANES_PATH` as a **TerminalPaneRegistry** (each `pair term` self-registers pane id + pid at startup; readers filter by pid liveness): zellij 0.44.3 omits `terminal_command` for `--direction`-created panes and the #118 tab-strip title is user-renamable, so neither is a usable signal. Never relative `move-focus right` — the id-based rule that fixed the #123 focus lockout survives the filler's deletion because it also targets a specific split half. `Alt+Shift+←`/`Alt+Shift+→` (#216) switch the right pane's TABS from any pane and leave focus where it is, so the draft stays the cockpit while the operator checks another tab. They are the first globals a pane handles ITSELF rather than by routing a Lua call into the draft — `GlobalBinding.HandledInPane` makes `DecideGlobal` withhold the `DraftLuaFunction` the Go executors branch on first, which is what stops tab switching from depending on nvim being alive. Delivery is `zellij action write --pane-id <half> <Alt+Left/Right bytes>` into the terminal's own pty, so `pair term` cannot tell a written chord from a pressed one and there stays exactly ONE implementation of tab switching; the right pane, already being that pane, calls the same `mux` method directly instead. **Pane resolution is sidecar-first, because `list-panes --json` is not cheap (#220).** On zellij 0.44.3 `zellij action list-panes --json` costs **590ms** against **22ms** for the plain form — it is `--json` alone; `--command`/`--state`/`--geometry` add nothing measurable — and `layoutcmd.OSRuntime.ListPanesJSON` runs it, so every action that resolved a pane paid it (`Alt+k` measured 671ms, the tab chord 631ms). The pane list is only needed for the zellij-focus tie-break, so `resolveFromSidecars(liveIDs, lastTerminal)` — pure, no Runtime, mirroring `draftroute.ValidateCachedDraftPane`'s shape — answers from the registry plus the recorded half when it can, and **declines** when it cannot: zero live ids, a record the registry cannot confirm, or two or more halves with no record. Only a decline spends the 590ms. The rule for a new caller: if you need a pane **ID**, resolve sidecar-first; if you need **geometry or the full pane set**, you must list panes, which is why `RunToggleFocused` still does. The fast path's guarantee is scoped — it agrees with `pickRightTerminal` whenever the registry is consistent with the pane report, and never returns an id the registry did not list; with an incomplete registry (a half that has not self-registered yet) and no recorded half, both paths are guessing. `procutil.Alive` is a guarded `syscall.Kill(pid, 0)` for the same reason: the registry filter calls it per line on an interactive path. Deployment shape worth knowing: these commands are spawned **per keypress**, so rebuilding `bin/pair` reaches a running session with no restart — unlike code compiled into the long-lived `pair term`/`pair wrap`/nvim, which does not. On the DELIVERY path (draft and agent panes) the half is chosen by the same `resolveRightTerminal` the focus jump uses, so `Alt+k` and `Alt+Shift+arrow` cannot disagree about which split half they mean. From INSIDE a split half the chord is handled locally and switches that half's own tabs regardless of the recorded id — correct, since the operator is looking at it, but it is why the guarantee is scoped to delivery rather than stated absolutely. Delivery is fire-and-forget: a terminal that exits between resolution and the write leaves a stale id and the write fails silently, which beats an error about a pane the operator just closed. These chords replaced `nav_boundary`, the draft's former history/queue Home-End jump, deleted by operator decision.
...
3. **Zellij and consumers.** Config pins `host_notification_protocol "osc9"`. Zellij converts the canonical envelope into `OSC9;pair: <message>`. Endpoint's registered adapter restores typed title `pair` and the full4096-byte body; generic OSC9 handling keeps its existing limit. Presenter serializes physical effects. Couch attributes hidden attention to the originating endpoint, suppresses focused attention badges, and does not replay notifications while switching/repainting. The old outer-TTY capture artifact remains launcher compatibility metadata and no longer authorizes notification writes.
     ...
     **Stdin Enter remap (per-harness profile).** One registry, `harnessTTYProfiles` (`cmd/internal/wrapcmd/harness_tty.go`), owns every harness's Return behavior; `profileForHarness` hands out a copy whose mutable keymap slices are the caller's own. A profile carries the send keymap, the overlay detector, a `composerGate` policy, and — for positively gated harnesses — a pure `recognize(terminalSnapshot) bool`. The normal convention matches pair's nvim draft pane: Enter = newline, Alt+Enter = send. For `claude` plain Enter becomes `\<CR>` (claude's portable "insert newline" sequence); Alt+Enter becomes a bare `\r` (send). Codex and Agy use LF for composer newline and CR for send. Muse's composer newline is its native Shift+Return sequence (`ESC [13;2u`), while Alt+Return becomes bare CR for submit. Qoder reuses claude's `\<CR>` newline with bare-CR send. Positive gating still protects the profiles' remaps: plain Enter rewrites to the harness's own `keymap.plainCR` only when the harness's recognizer confirms a live composer in the current screen snapshot; otherwise it passes through as a bare `\r`, which the agent reads as the Enter key (submit / picker confirm). The gate policy's zero value is `composerGateUnknown`, so an absent or corrupt profile fails closed to bare CR rather than authorizing a remap. Their submit chord must still collapse to `\r`, not a modified `ESC CR`. The same keymap carries `altBS`: Alt+Backspace (legacy `\x1b\x7f` or KKP `\x1b[127;3u`, the same two-protocol shape as Alt+Enter) rewrites to **Ctrl+U** (`0x15`, kill-to-line-start) for every agent — so Alt+Delete in the agent pane matches the agent's Cmd+Delete and the draft pane's Alt+Delete. A lone `0x7f` (plain Backspace) isn't ESC-prefixed, so it passes through untouched. Opt out of the whole remap with `PAIR_WRAP_REMAP_RETURN=0`. Muse's newline is a Kitty keyboard protocol key (`ESC [13;2u`, Shift+Return), so it carries a precondition the other profiles don't: it parses only while Muse keeps progressive enhancement pushed. Because every other assertion reads its expectation *from* the profile, that one is checked against the capture itself — `assertKittyKeyboardPrecondition` fails any harness with a KKP-encoded `plainCR` whose own capture pushes no flags, in both the frozen replay and the live check. Left unchecked the failure is silent in the worst way: Return does nothing and `return-remap` still reports `fired`.
     ...
     *Overlay layer (negative override).* Each profile registers an overlay detector; when one fires, `pickerActive` is set and the next plain Enter emits a bare `\r` only, then the flag clears. Claude uses the stable `OSC 777;notify;Claude Code;Claude needs your permission` body. Codex question prompts use `OSC 9;Plan mode prompt:...`; other Codex pickers fall back to stripped visible output plus a short text carryover watching `codexPickerMarkers` labels such as `Use session directory (` / `Use current directory (`, `Press enter to continue`, and `Press enter to confirm or esc to go back`. Qoder's two picker families (permission and question) take the same stripped-text path over `qoderPickerMarkers` (`Rejectandtypesomething`, `Enterselect`, …), plus one split-integrity refinement: the detector also scans the raw rolling buffer, because a chunk boundary inside a marker's style run leaves a truncated escape verbatim in the per-chunk strip (replaying `selection.raw` split at 6426 cut `\x1b[23m` mid-`Enterselect`) while the byte-contiguous window cannot be corrupted that way. Detection and consumption share `overlayMu`, so a newly detected overlay can never be erased by an older Enter. The carryover is cleared when the confirming Enter is consumed so stale picker text cannot re-arm the flag.

     *Composer layer (positive gate).* A single shared terminal model (`terminal_model.go`, x/vt) consumes raw PTY bytes and publishes immutable `terminalSnapshot` values; per-harness recognizers in `composer_recognizers.go` are pure functions over those snapshots, replacing the retired per-agent byte trackers. `codexComposerActive` requires a **bold** U+203A at column 0 reachable from the cursor through painted continuation rows — Codex marks menu selections with the same glyph unemphasized and parks the cursor on its status line mid-paint, so both are rejected. `museComposerActive`, `claudeComposerActive` and `qoderComposerActive` register specs on one `ruledBoxComposerActive` predicate: a prompt glyph forming the first row inside a pair of rule rows, with the cursor anywhere in the box — anchoring on the *enclosing* rules rather than adjacent ones is what lets a composer grow past one line. The spec carries the axes on which the harnesses differ; Qoder's are `promptCol` (its glyph sits at column 1, `qoderPromptCol` being the single authority) and `allowHiddenCursor` (Qoder hides the system cursor behind one it paints itself; the zero value requires the visible cursor, so every other spec fails closed). Muse qualifies its prompt as a non-faint glyph from `musePromptGlyphs` between rules that agree on faintness — one authority, read by both the Return gate and orientation's auto-submit gate, because a glyph one accepts and the other refuses is a state where Return inserts a newline into a composer orientation will not submit into. The set is the chevron family (`⟩ › ❯ >`); selection markers (`!`, `●`, `▶`, `▸`) are deliberately excluded, and `muse/*/menu.raw` is why the exclusion costs nothing — Muse paints its slash menu below the box with column 0 blank, so it never reuses the prompt glyph as a highlight. Claude's spec is deliberately glyph- and colour-agnostic, requiring only a non-rule glyph at column 0 between two `─` rules that share a foreground, because Claude repaints both glyph and rule colour per input mode — bash mode is `!` with pink rules where the default is `❯` with grey — and Claude is the default agent, so a false negative submits a half-written draft. Qoder registers the ruled-box shape a third time: its prompt glyph (`>` in default mode, `*` in yolo — one `qoderPromptGlyphs` authority, shared with orientation's auto-submit gate) sits at column 1, its hidden system cursor is admitted via `allowHiddenCursor`, and colour agreement between the two rules is not required — Qoder paints top and bottom in different greys. `agyComposerActive` requires the cursor inside a coherent box: a `>` prompt column enclosed by two `─` border rows that both span the cursor column. If the recognizer declines, or the snapshot is unknown, plain Return passes through as bare CR — so new harness menus select on plain Enter without enumerating every footer. Resize is a latched transaction: authorization stays closed from validation until a complete successful resize commits, and a failed PTY resize leaves it closed. Codex image attach uses a stronger local signal: Alt+i arms pair-wrap capture immediately before Ctrl+V, and that capture arm also enables the next-Enter overlay bypass. Known edge inherited from the one-shot overlay design: dismissing an overlay without Enter leaves the flag set until the next plain Enter.

     *Conformance.* `cmd/internal/wrapcmd/testdata/tty/<agent>/<version>/` holds literal bytes captured from the real CLI through a bounded PTY seam, with a `metadata.json` recording version, argv, capture time, and SHA-256 per raw file. `composer.raw` must remap to the harness's own `keymap.plainCR` (LF for Codex/Agy, Muse Shift+Return, backslash-CR for Claude and Qoder — the expectation is read from the profile, not restated); an `overlay.raw` must stay bare CR. `TestHarnessTTYFixtureConformance` replays each fixture through the production proxy at every split from 0 to len and requires the recognizer, overlay arming, and `emitPlainCR` bytes to match the unsplit baseline, so chunk boundaries can never change what Return does. `PAIR_LIVE_HARNESS=<agent>` runs the live checks against the installed CLI; they assert behavior, not byte identity, because real output embeds per-account and per-moment content and harnesses self-update. On drift they print the exact recapture destination, and `PAIR_LIVE_CAPTURE_OUT` writes the new prefix atomically. **These live checks are manual and opt-in — nothing schedules them.** Harness drift is therefore caught when someone runs them, not automatically; the frozen-fixture replay is the part that runs on every `go test`. A harness whose gate has no captured declining state must be named in `ttyFixtureNegativeGaps` with the reason; `TestHarnessTTYFixtureConformance` fails for any positively gated harness that is neither covered by an `overlay.raw` nor acknowledged there, and fails again once an acknowledgment outlives its gap. Two sibling ledgers work the same way and expire the same way. `ttyFixtureDiscriminationGaps` covers harnesses whose declining states don't prove the gate separates a composer from a picker painted in the same shape. `ttyFixtureReactionGaps` covers the question no replay can answer: a capture proves what the *wrapper* emits, never what the harness then does with it — so every positively gated harness, the composer screen included, must either press Return in a driven scenario or record what is unproven. A comment asserting the outcome does not count, which is the rule that caught four such claims at once (every harness, as it turns out: no Return has ever been pressed on a captured screen). All three ledgers now expire on the same shape, and on a property the scenario *declares* rather than one inferred from a keystroke — `pressesReturn` for a reaction, `discriminating` for a negative painted in the composer's own shape — because a retirement oracle that approximates its property will retire a gap that still exists. Where the declared property has components the model can check, the oracle conjoins them (a `discriminating` screen must be one the gate declines AND must be captured); only the genuinely uncheckable residue is honor-system, and each field's doc says which part that is. A fourth ledger, `ttyFixtureEnvironmentGaps`, covers a harness whose capture cannot avoid embedding the capture machine's filesystem: a fixture must contain no absolute home path, since one differs on recapture elsewhere for reasons unrelated to harness drift, and Muse currently cannot comply — it prints the absolute path of this repo's ignored `CLAUDE.md` anywhere inside the checkout, and outside it demands workspace trust instead of painting a composer. **Retention:** the newest version directory per harness, plus any older one a test names by path; every fixture replays at every byte split on every `go test` (one three-screen capture set is ~45% of that suite's runtime), so a superseded directory is deleted when its replacement lands. Muse's fixture also carries `shortcuts.raw`, the `?` sheet where Muse prints its own key contract — "shift + enter for newline", "enter to submit message" — the documentary basis for its inverted keymap. `assertKittyKeyboardPrecondition` additionally requires a KKP-encoded `plainCR` to be backed by a push whose flag word sets the disambiguate bit, since `CSI > 0 u` is a push that disables every enhancement.
     ...
     **Native notification normalizer.** Parsing every OSC `<Ps>;<body>` and discriminating is essential — naive "any BEL → emit" over-fires constantly because claude (and similar agents) update OSC 0 (window title) every second with a spinner. The chunk-safe rewriter:
     ...
     A pure turn-lifecycle reducer sits before the canonical emit boundary. User submission, progress working, transcript starts, and a bare return that reached the agent open generations; native/marker completions, explicit progress stop after a short grace, and activity-gated watchdog expiry converge there and notify at most once per generation. The idle floor is the one deliberate exception: it is an alert rather than a terminal. A generation carries at most one completion, and one alert per idle *epoch* — a bare return re-arms a spent floor on a fresh epoch, so a turn the operator interacts with can alert more than once. Two resettable timers are owned by the master proxy loop, with tokenized expiries preventing stale timers from completing a later turn. The existing 0.5s emit limiter remains transport throttling rather than semantic deduplication. Empirically: claude emits `OSC 777;notify;Claude Code;Claude is waiting for your input` after ~60s of idle waiting — that actionable signal enters the same lifecycle.

     **The idle floor (#171).** The watchdog is armed only by observed *activity*, so a turn whose activity is never recognized gets no deadline at all — and measurement found that to be the common case, not the edge: claude's progress OSC arrives as sparse matched working/stopped pairs, absent from most turns, so `ActivitySeen` usually stays false. The floor is the second timer: armed whenever a turn is open, unreported, and has not already alerted, reset by every output chunk, and expiring into `ObservationIdleExpired`. Its epoch (`IdleToken`) is minted when a turn opens and again whenever a bare return re-arms a spent floor — never per chunk, so output pushes the deadline out without changing which epoch an expiry belongs to. It is an **alert, not a completion** — it notifies "no agent output for Ns" and leaves the turn open, because a silence does not know the turn ended; a genuine completion afterwards still notifies. Arming is owned by `syncIdleTimer` beside `syncLifecycleTimer`, so it can never drift from lifecycle state, and the expiry branch drains queued boundaries first, applying a completion that was published but not yet reduced before the expiry. The epoch that expired is snapshotted before that drain: an opener among the drained events mints a new turn, and reading the token afterwards would alert against a turn that had just opened and consume its floor. Turn openers cover both stdin configurations: with a remap profile, Alt+Enter and a composer-inactive bare CR publish through `translateChunk`; without one — `PAIR_WRAP_REMAP_RETURN=0`, or any agent outside `harnessTTYProfiles` — `passThroughChunk` publishes on a CR, since those bytes reach the agent verbatim. A bare CR inside an open turn re-arms a floor that already fired, so answering a menu starts a new attention window rather than leaving that turn uncovered. `PAIR_WRAP_IDLE_S` sets the interval; `0` disables. It arms for every agent — the mode gate that previously bound it to a notify-mode value nothing was ever assigned is gone.
...
- **Statusline cheatsheet (right-aligned, progressive disclosure).** `PAIR_CHEATS` lists `Alt+h help`, `Alt+⏎ send`, `Alt+q queue`, `Alt+x quit`, `Alt+d detach` in priority order. `pair_compose_statusline` measures the variable left segment (history/queue/position cluster), reserves a 6-cell minimum gap, and accumulates as many cheat entries as fit in the remaining columns — Alt+h is always the last entry to drop. Spinner takes the right slot when active (vim only honors a single `%=` per statusline). The minimized rung asks the operator to click the draft before using Alt+↑ to expand it, with 4 leading spaces so the terminal cursor (which lands on the statusline row when the buffer has zero visible lines) sits on whitespace rather than the hint text.
- **Alt+x / Alt+d / Alt+n / Shift+Alt+N confirm modals.** `PairConfirmQuit` / `PairConfirmDetach` / `PairConfirmRestart` / `PairConfirmAgentRestart` invoke `pair quit` / `zellij action detach` / `pair restart` / `pair agent restart` after a Y/N modal that defaults to No. All four run their command through `_G._pair_lifecycle.run` (`nvim/lifecycle_command.lua`, `pair#284`): **every confirmed lifecycle command reports its own failure**, notifying the command's output — or its exit status when it printed nothing — at ERROR. Success says nothing, since quit/detach/restart end the session. The wiring is pinned by `tests/lifecycle-command-nvim-test.sh`, which also fails on a direct `vim.fn.system` call. All four are wrapped in `pair_ensure_visible_then`, which auto-grows out of `minimized` (calls `PairLayoutBigger` and defers the modal 100ms) so the prompt renders on visible rows. Alt+n owns the whole-workbench restart path; Shift+Alt+N only replaces the supervised agent child.
...
Implementation in `nvim/init.lua`: see helpers grouped under `is_dirty_history_slot`, `autosave_current_slot`, `leave_dirty_history`, `go_to`, `nav_left`/`nav_right`, `queue_current`, `delete_current_queue_item`, `forget_all`, plus the `queue_*` file ops. State lives in module-local `nav = { pos, baseline }` — `pos` is `'*'` or `{ kind='history'|'queue', n=N }`.
...
- **Trigger** — `pair-wrap`'s turn-end detection (`emitOuter`, the agent-agnostic
  notify sink: marker-regex for claude, native OSC for codex/agy, and the
  always-armed idle floor for every agent). On
