...
- `pair <agent>` without `--` uses the same work picker with explicit driver intent. Same-agent live rows are attachable. Different-agent live rows are displayed but unavailable because Pair will not attach the requested agent to another live driver. Different-agent historical rows resolve to the existing continuation-backed create path only when `ResolveContinuationDoc(tag)` finds a matching doc; otherwise the launcher refuses before mutation. This gives the user a visible cross-agent switch affordance without reviving the abandoned live takeover coordinator.
