...
Detection of attached-vs-detached uses `zellij --session NAME action list-clients`, which prints a header plus one row per connected client. Zero rows = detached. That call costs about 250 ms against a real detached session (`pair#228`, `cmd/probes/reattachcost`), and `list-sessions` carries no client information, so one snapshot routine (`ZellijSource.snapshot`) serves three forms by who needs what. The full form asks every session and is used by bare `pair`'s picker and `pair list`. `SnapshotSessionsContext` asks only named sessions and is used by couch's detached proof. `LivenessContext` asks none, reporting `SessionLive`, and is used by `pair resume` and couch's registration poll. `DecideLaunch`'s `launchShape` switch is what decides which form `runOnce` takes.
