...
**Birth gate (`pair#287`).** The poller makes no zellij call until the agent pane's sidecar (`Paths.Pane(agent)`) exists. It stats every 250 ms, bounded by `StartupGrace` (30 s), and exits without touching zellij if the pane never appears.
