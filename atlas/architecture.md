...
3. **Zellij and consumers.** Config pins `host_notification_protocol "osc9"`. Zellij converts the canonical envelope into `OSC9;pair: <message>`. Endpoint's registered adapter restores typed title `pair` and the full4096-byte body; generic OSC9 handling keeps its existing limit. Presenter serializes physical effects. Couch attributes hidden attention to the originating endpoint, suppresses focused attention badges, and does not replay notifications while switching/repainting. The old outer-TTY capture artifact remains launcher compatibility metadata and no longer authorizes notification writes.
     ...
     **Draft submit inside a paste window (#266).** The draft's `write-chars` body reaches the wrapper as a bracketed paste, and Zellij can deliver the following `send-keys "Alt Enter"` *before* that paste's close marker. `translateChunk`'s in-paste branch therefore looks for an Alt+Enter chord (legacy `\x1b\r` or KKP `\x1b[13;3u`) ahead of `bpEnd`, emits `keymap.altCR`, publishes `ObservationUserSubmission`, and holds back a split Alt partial across the chunk boundary; every other byte inside the window still forwards literally, and a plain `\r` inside a paste is never remapped. Without that out-of-band recognition the submit is forwarded as literal composer text and the draft sits unsent. The cost is deliberate and bounded to this one chord: a paste whose own payload contains Alt+Enter reads as a submit.
