...
- `copy_command "pair clip copy-on-select"` — on every selection finalize (mouse-up after drag), zellij pipes the selected text to this binary, which **mirrors it to the OS clipboard and stops** (#125). It used to also detach an orchestrator that auto-pasted the selection into the draft; that was removed as distracting, and the rest of the chain is now unwired (see #126). `copy_command` replaces zellij's default OS-clipboard write, so the binary does that part too. Resolved by PATH (which the launcher populated).
...
### `copy-on-select` — zellij copy_command wrapper (mirror-only since #125)
...
**Since #125 only the hook runs.** The orchestrator below is unwired — no production caller — and is kept as the machinery #126 will bind to a deliberate quote-paste keybind. The split it describes is still accurate for what the orchestrator *does*; it just isn't reached from a selection anymore.
...
- `PairPasteQuote()` (mapped to `<C-_>` in insert mode; **unwired since #125 — no production caller**, since the copy-on-select hand-off that sent Ctrl-_ / ASCII 31 is gone; #126 will give it a deliberate trigger): reads the raw selection from `$PAIR_DATA_DIR/quote-<tag>` and dispatches on cursor column.
...
- `quote-<tag>` — transient hand-off file written by `pair clip clipboard-to-pane` and read by nvim's `PairPasteQuote()`. **Unwired since #125 — nothing writes it** (a selection no longer hands off); #126 will restore a writer via a deliberate keybind.
